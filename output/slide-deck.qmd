...
With this timeline, we have the information about what has changed, but also
when (for example, seeing the timeline we may know what results are affected if
an analysis script has changed, and whether they need to be updated),...
...
Then, if we are satisfied with the new changes, and maybe gather enough
consensus, we can integrate those changes, or dismiss them if we don't want to
make them part of our main outome.
:::
...
However, one important thing, whether we merge our parallel history to the main
one or not: Nothing is ever lost (unless we really wanted it to, of course).
:::
...
the tools to avoid these errors. And if they happen, a certain version can
always be deleted from the history, as I told you before.
:::
...
So after all these disadvantages, one may wonder if it is worth using version
control systems.
:::
